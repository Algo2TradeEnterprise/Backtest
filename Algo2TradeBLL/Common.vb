...
#Region "Public Functions"
    ...
    Public Function GetExpiry(ByVal tableName As DataBaseTable, ByVal tradingSymbol As String, ByVal currentDate As Date) As Date
        Dim ret As Date = Date.MinValue
        Dim dt As DataTable = Nothing
        Dim conn As MySqlConnection = OpenDBConnection()
        Dim cm As MySqlCommand = Nothing
        ...
        Select Case tableName
            Case DataBaseTable.Intraday_Cash, DataBaseTable.EOD_Cash
                cm = New MySqlCommand("SELECT `EXPIRY` FROM `active_instruments_cash` WHERE `TRADING_SYMBOL`=@trd AND `AS_ON_DATE`=@sd", conn)
            Case DataBaseTable.Intraday_Currency, DataBaseTable.EOD_Currency
                cm = New MySqlCommand("SELECT `EXPIRY` FROM `active_instruments_currency` WHERE `TRADING_SYMBOL`=@trd AND `AS_ON_DATE`=@sd", conn)
            Case DataBaseTable.Intraday_Commodity, DataBaseTable.EOD_Commodity
                cm = New MySqlCommand("SELECT `EXPIRY` FROM `active_instruments_commodity` WHERE `TRADING_SYMBOL`=@trd AND `AS_ON_DATE`=@sd", conn)
            Case DataBaseTable.Intraday_Futures, DataBaseTable.EOD_Futures
                cm = New MySqlCommand("SELECT `EXPIRY` FROM `active_instruments_futures` WHERE `TRADING_SYMBOL`=@trd AND `AS_ON_DATE`=@sd", conn)
        End Select
