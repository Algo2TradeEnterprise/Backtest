...
#Region "CRUDEOIL EOD"
            ...
            '                                                  lastTradeEntryTime:=TimeSpan.Parse("22:00:00"),
            '                                                  eodExitTime:=TimeSpan.Parse("22:30:00"),
            '                                                  tickSize:=tick,
            '                                                  marginMultiplier:=margin,
            '                                                  timeframe:=1,
            '                                                  heikenAshiCandle:=False,
            ...
            '                                                  usableCapital:=8000,
            '                                                  minimumEarnedCapitalToWithdraw:=15000,
...
#Region "Favourable fractal breakout 2"
            'Using backtestStrategy As New MISGenericStrategy(canceller:=_canceller,
            '                                                  exchangeStartTime:=TimeSpan.Parse("09:15:00"),
            '                                                  exchangeEndTime:=TimeSpan.Parse("15:29:59"),
            '                                                  tradeStartTime:=TimeSpan.Parse("9:45:00"),
...
#Region "High Low EMA"
            ...
            '    With backtestStrategy
            '        .StockFileName = Path.Combine(My.Application.Info.DirectoryPath, "ATR Stocklist 1.csv")
            ...
            '        .RuleEntityData = New HighLowEMAStrategyRule.StrategyRuleEntities With
            '            {
            '                .TargetMultiplier = 2
            '            }
            ...
            '    Dim filename As String = String.Format("High Low EMA")

            '    Await backtestStrategy.TestStrategyAsync(startDate, endDate, filename).ConfigureAwait(False)
            'End Using
#End Region

#Region "Top Gainer Looser"
            'Using backtestStrategy As New MISGenericStrategy(canceller:=_canceller,
            '                                                  exchangeStartTime:=TimeSpan.Parse("09:15:00"),
            '                                                  exchangeEndTime:=TimeSpan.Parse("15:29:59"),
            '                                                  tradeStartTime:=TimeSpan.Parse("9:20:00"),
            ...
            '    With backtestStrategy
            '        .StockFileName = Path.Combine(My.Application.Info.DirectoryPath, "Top Gainer Losser of Every Slab.csv")
            ...
            '    Dim filename As String = String.Format("Top Gainer Looser Output")
...
#Region "Pair Strategy"
            ...
            '                                                  exchangeEndTime:=TimeSpan.Parse("15:29:59"),
            '                                                  tradeStartTime:=TimeSpan.Parse("9:20:00"),
            '                                                  lastTradeEntryTime:=TimeSpan.Parse("9:30:00"),
            '                                                  eodExitTime:=TimeSpan.Parse("15:15:00"),
            '                                                  tickSize:=tick,
            '                                                  marginMultiplier:=margin,
            '                                                  timeframe:=1,
...
#Region "Low Stoploss Slab"
            ...
            '                    .ExitOnOverAllFixedTargetStoploss = True
            '                    .OverAllProfitPerDay = Decimal.MaxValue
            '                    .OverAllLossPerDay = overallLoss

            '                    .TypeOfMTMTrailing = trlngType
            '                    .MTMSlab = Math.Abs(.OverAllLossPerDay)
            '                    .MovementSlab = .MTMSlab / 2
            '                    .RealtimeTrailingPercentage = 50
            '                End With
            ...
            '                Dim filename As String = String.Format("Low SL Slab,TF {0},OvrLs {1},TrlngTyp {2}",
            '                                                       backtestStrategy.SignalTimeFrame, backtestStrategy.OverAllLossPerDay, backtestStrategy.TypeOfMTMTrailing)
...
#Region "Low Stoploss Fractal"
            'For tgtMul As Integer = 3 To 5
            '    For ovralLoss As Decimal = -6000 To -12000 Step -3000
            '        For trlng As Integer = 2 To 4 Step 2
            '            Dim lastMvmntSlab As Decimal = 66.6
            '            If trlng = 4 Then lastMvmntSlab = 50
            '            For trlngMvmntSlabPer As Decimal = 50 To lastMvmntSlab Step 16.6
            '                Using backtestStrategy As New MISGenericStrategy(canceller:=_canceller,
            '                                                                    exchangeStartTime:=TimeSpan.Parse("09:15:00"),
            '                                                                    exchangeEndTime:=TimeSpan.Parse("15:29:59"),
            '                                                                    tradeStartTime:=TimeSpan.Parse("9:16:00"),
            ...
            '                        .RuleEntityData = New LowStoplossFractalStrategyRule.StrategyRuleEntities With
            '                        {
            '                         .MaxStoplossPerTrade = -1200,
            ...
            '                        .TypeOfMTMTrailing = trlng
            '                        .MTMSlab = Math.Abs(.OverAllLossPerDay)
            '                        .MovementSlab = .MTMSlab * trlngMvmntSlabPer / 100
            '                        .RealtimeTrailingPercentage = 50
            '                    End With

            '                    Dim ruleData As LowStoplossFractalStrategyRule.StrategyRuleEntities = backtestStrategy.RuleEntityData
            '                    Dim filename As String = String.Format("Low SL Fractal TgtMul {0},OvrLs {1},TrlngTyp {2},MvmntSlab {3}({4})",
            '                                                           tgtMul, ovralLoss, backtestStrategy.TypeOfMTMTrailing, Math.Round(backtestStrategy.MovementSlab, 0), trlngMvmntSlabPer)
...
#Region "Two candle high low breakout"
            'Using backtestStrategy As New MISGenericStrategy(canceller:=_canceller,
            '                                                exchangeStartTime:=TimeSpan.Parse("09:15:00"),
            '                                                exchangeEndTime:=TimeSpan.Parse("15:29:59"),
            '                                                tradeStartTime:=TimeSpan.Parse("9:17:00"),
            ...
            '    Dim filename As String = String.Format("Two Cndl HL Brkout")
...
#Region "Average Price Drop"
            ...
            '        .RuleEntityData = New AveragePriceDropContinuesStrategyRule.StrategyRuleEntities With
            '            {.BuyAtEveryPriceDropPercentage = 1,
            '             .QuantityType = AveragePriceDropContinuesStrategyRule.TypeOfQuantity.GP}
            ...
            '        .TickBasedStrategy = True
            '    End With
            '    Dim ruleData As AveragePriceDropContinuesStrategyRule.StrategyRuleEntities = backtestStrategy.RuleEntityData
            '    Dim filename As String = String.Format("AvrgPrcDrp,Quantity Type {3} {0}_{1}_{2}",
            '                                           Now.Hour, Now.Minute, Now.Second, ruleData.QuantityType)
            '    Await backtestStrategy.TestStrategyAsync(startDate, endDate, filename).ConfigureAwait(False)
            'End Using
#End Region
...
#Region "HK RSI CNC"
            Using backtestStrategy As New CNCEODGenericStrategy(canceller:=_canceller,
                ...
                With backtestStrategy
                    ...
                    .RuleEntityData = New HKRSIContinuesStrategyRule.StrategyRuleEntities With
                        {
                         .QuantityType = HKRSIContinuesStrategyRule.TypeOfQuantity.AP,
                         .BuyAtBelowRSI = 3
                        }
                    ...
                    .TickBasedStrategy = True
                End With
                Dim ruleData As HKRSIContinuesStrategyRule.StrategyRuleEntities = backtestStrategy.RuleEntityData
                Dim filename As String = String.Format("HK RSI,Quantity Type {3} {0}_{1}_{2}",
                                                       Now.Hour, Now.Minute, Now.Second, ruleData.QuantityType)
                Await backtestStrategy.TestStrategyAsync(startDate, endDate, filename).ConfigureAwait(False)
            End Using
#End Region
