...
#End Region
    ...
    Public Overrides Async Function IsTriggerReceivedForPlaceOrderAsync(currentTick As Payload) As Task(Of Tuple(Of Boolean, List(Of PlaceOrderParameters)))
        ...
        If currentMinuteCandle IsNot Nothing AndAlso currentMinuteCandle.PreviousCandlePayload IsNot Nothing AndAlso
            ...
            Dim signal As Tuple(Of Boolean, Decimal, Decimal, Payload, Trade.TradeExecutionDirection, String) = GetEntrySignal(currentMinuteCandle, currentTick)
    ...
    Private Function GetEntrySignal(ByVal currentCandle As Payload, ByVal currentTick As Payload) As Tuple(Of Boolean, Decimal, Decimal, Payload, Trade.TradeExecutionDirection, String)
        ...
        If currentCandle IsNot Nothing AndAlso currentCandle.PreviousCandlePayload IsNot Nothing Then
            Dim vwap As Decimal = _vwapPayload(currentCandle.PreviousCandlePayload.PayloadDate)
            ...
            If currentCandle.PreviousCandlePayload.Low > vwap Then
                Dim entryPrice As Decimal = currentTick.Open
                Dim vwapSL As Decimal = ConvertFloorCeling(entryPrice - vwap, _parentStrategy.TickSize, RoundOfType.Celing)
                If atr > vwapSL Then
                    ret = New Tuple(Of Boolean, Decimal, Decimal, Payload, Trade.TradeExecutionDirection, String)(True, entryPrice, entryPrice - atr, currentCandle.PreviousCandlePayload, Trade.TradeExecutionDirection.Buy, "ATR")
                Else
                    ret = New Tuple(Of Boolean, Decimal, Decimal, Payload, Trade.TradeExecutionDirection, String)(True, entryPrice, entryPrice - vwapSL, currentCandle.PreviousCandlePayload, Trade.TradeExecutionDirection.Buy, "VWAP")
                End If
            ElseIf currentCandle.PreviousCandlePayload.High < vwap Then
                Dim entryPrice As Decimal = currentTick.Open
                Dim vwapSL As Decimal = ConvertFloorCeling(vwap - entryPrice, _parentStrategy.TickSize, RoundOfType.Celing)
                If atr > vwapSL Then
                    ret = New Tuple(Of Boolean, Decimal, Decimal, Payload, Trade.TradeExecutionDirection, String)(True, entryPrice, entryPrice + atr, currentCandle.PreviousCandlePayload, Trade.TradeExecutionDirection.Sell, "ATR")
                Else
                    ret = New Tuple(Of Boolean, Decimal, Decimal, Payload, Trade.TradeExecutionDirection, String)(True, entryPrice, entryPrice + vwapSL, currentCandle.PreviousCandlePayload, Trade.TradeExecutionDirection.Sell, "VWAP")
                End If
            End If
        End If
        Return ret
