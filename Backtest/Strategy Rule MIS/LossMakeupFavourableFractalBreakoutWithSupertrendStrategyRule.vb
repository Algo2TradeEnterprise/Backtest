...
Public Class LossMakeupFavourableFractalBreakoutWithSupertrendStrategyRule
...
#End Region
    ...
    Public Overrides Sub CompletePreProcessing()
        ...
        Indicator.Supertrend.CalculateSupertrend(50, 5, _signalPayload, _supertrendPayload, _supertrendColorPayload)
    End Sub
    ...
    Private Function GetSignalCandle(ByVal candle As Payload, ByVal currentTick As Payload, ByVal direction As Trade.TradeExecutionDirection) As Tuple(Of Boolean, Decimal)
        ...
        If candle IsNot Nothing AndAlso candle.PreviousCandlePayload IsNot Nothing Then
            If direction = Trade.TradeExecutionDirection.Buy AndAlso _supertrendColorPayload(candle.PayloadDate) = Color.Green Then
                ...
                End If
            ElseIf direction = Trade.TradeExecutionDirection.Sell AndAlso _supertrendColorPayload(candle.PayloadDate) = Color.Red Then
    ...
    Private Function GetStoplossPrice(ByVal direction As Trade.TradeExecutionDirection, ByVal signalCandle As Payload, ByVal entryPrice As Decimal, ByVal buffer As Decimal) As Tuple(Of Decimal, String)
        ...
        If signalCandle IsNot Nothing Then
            If direction = Trade.TradeExecutionDirection.Buy Then
                ...
                Dim fractalSLPoint As Decimal = (entryPrice - _fractalLowPayload(signalCandle.PayloadDate)) + 2 * buffer
                Dim supertrendSLPoint As Decimal = (entryPrice - ConvertFloorCeling(_supertrendPayload(signalCandle.PayloadDate), _parentStrategy.TickSize, RoundOfType.Floor)) + 2 * buffer
                Dim maxATRSL As Decimal = ConvertFloorCeling(atr * _userInputs.MaximumStoplossATRMultipler, _parentStrategy.TickSize, RoundOfType.Celing) + 2 * buffer
                Dim slPoint As Decimal = maxATRSL
                Dim slRemark As String = "Max ATR"
                If fractalSLPoint > 0 AndAlso fractalSLPoint < slPoint Then
                    slPoint = fractalSLPoint
                    slRemark = "Fractal"
                End If
                If supertrendSLPoint > 0 AndAlso supertrendSLPoint < slPoint Then
                    slPoint = fractalSLPoint
                    slRemark = "Supertrend"
                End If

                Dim minATRSL As Decimal = ConvertFloorCeling(atr * _userInputs.MinimumStoplossATRMultipler, _parentStrategy.TickSize, RoundOfType.Celing) + 2 * buffer
                If slPoint < minATRSL Then
                    slPoint = minATRSL
                    slRemark = "Min ATR SL"
                End If
                ret = New Tuple(Of Decimal, String)((entryPrice + buffer) - slPoint, slRemark)
            ElseIf direction = Trade.TradeExecutionDirection.Sell Then
                Dim atr As Decimal = ConvertFloorCeling(_atrPayload(signalCandle.PayloadDate), _parentStrategy.TickSize, RoundOfType.Celing)
                Dim fractalSLPoint As Decimal = (_fractalHighPayload(signalCandle.PayloadDate) - entryPrice) + 2 * buffer
                Dim supertrendSLPoint As Decimal = (ConvertFloorCeling(_supertrendPayload(signalCandle.PayloadDate), _parentStrategy.TickSize, RoundOfType.Floor) - entryPrice) + 2 * buffer
                Dim maxATRSL As Decimal = ConvertFloorCeling(atr * _userInputs.MaximumStoplossATRMultipler, _parentStrategy.TickSize, RoundOfType.Celing) + 2 * buffer
                Dim slPoint As Decimal = maxATRSL
                Dim slRemark As String = "Max ATR"
                If fractalSLPoint > 0 AndAlso fractalSLPoint < slPoint Then
