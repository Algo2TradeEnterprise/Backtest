...
Public Class AnchorSatelliteLossMakeupHKFuturesStrategyRule
...
#Region "Entity"
    Public Class StrategyRuleEntities
        ...
        Public LossMakeupWithMoreThanDoubleQuantity As Boolean
    End Class
#End Region
    ...
    Public Overrides Sub CompletePreProcessing()
        ...
        Dim loss As Decimal = _parentStrategy.CalculatePL(_tradingSymbol, _firstCandleOfTheDay.Open, _firstCandleOfTheDay.Open - _slPoint, _quantity, Me.LotSize, _parentStrategy.StockType)
        Me.MaxProfitOfThisStock = Math.Abs(loss)
        _targetPoint = _parentStrategy.CalculatorTargetOrStoploss(_tradingSymbol, _firstCandleOfTheDay.Open, _quantity, Me.MaxProfitOfThisStock, Trade.TradeExecutionDirection.Buy, _parentStrategy.StockType) - _firstCandleOfTheDay.Open
    End Sub
        ...
        If currentMinuteCandlePayload IsNot Nothing AndAlso currentMinuteCandlePayload.PreviousCandlePayload IsNot Nothing AndAlso
            ...
            If Not _parentStrategy.IsTradeOpen(currentTick, Trade.TypeOfTrade.MIS, Trade.TradeExecutionDirection.Buy) AndAlso
                ...
                If signal IsNot Nothing AndAlso signal.Item1 Then
                    ...
                    If pl < 0 Then
                        If _userInputs.LossMakeupWithMoreThanDoubleQuantity Then
                            Dim tempQty As Integer = _parentStrategy.CalculateQuantityFromTargetSL(_tradingSymbol, entryPrice, entryPrice + targetPoint, Math.Abs(pl), _parentStrategy.StockType)
                            quantity = Math.Ceiling(tempQty / Me.LotSize) * Me.LotSize
                            targetPoint = _parentStrategy.CalculatorTargetOrStoploss(_tradingSymbol, entryPrice, quantity, Math.Abs(pl), Trade.TradeExecutionDirection.Buy, _parentStrategy.StockType) - entryPrice
                        Else
                            quantity = Math.Abs(GetStockPotentialQuantity(currentMinuteCandlePayload, Trade.TradeExecutionDirection.Buy))
                            targetPoint = _parentStrategy.CalculatorTargetOrStoploss(_tradingSymbol, entryPrice, quantity, Math.Abs(pl), Trade.TradeExecutionDirection.Buy, _parentStrategy.StockType) - entryPrice
                        End If
                ...
                If signal IsNot Nothing AndAlso signal.Item1 Then
                    ...
                    If pl < 0 Then
                        If _userInputs.LossMakeupWithMoreThanDoubleQuantity Then
                            Dim tempQty As Integer = _parentStrategy.CalculateQuantityFromTargetSL(_tradingSymbol, entryPrice, entryPrice + targetPoint, Math.Abs(pl), _parentStrategy.StockType)
                            quantity = Math.Ceiling(tempQty / Me.LotSize) * Me.LotSize
                            targetPoint = entryPrice - _parentStrategy.CalculatorTargetOrStoploss(_tradingSymbol, entryPrice, quantity, Math.Abs(pl), Trade.TradeExecutionDirection.Sell, _parentStrategy.StockType)
                        Else
                            quantity = Math.Abs(GetStockPotentialQuantity(currentMinuteCandlePayload, Trade.TradeExecutionDirection.Sell))
                            targetPoint = entryPrice - _parentStrategy.CalculatorTargetOrStoploss(_tradingSymbol, entryPrice, quantity, Math.Abs(pl), Trade.TradeExecutionDirection.Sell, _parentStrategy.StockType)
                        End If
    ...
    Private Function GetStockPotentialQuantity(ByVal currentMinutePayload As Payload, ByVal direction As Trade.TradeExecutionDirection) As Integer
