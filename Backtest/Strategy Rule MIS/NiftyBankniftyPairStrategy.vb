...
Public Class NiftyBankniftyPairStrategy
    ...
    Private ReadOnly _tradeEntryTimeList As Dictionary(Of Date, Trade.TradeExecutionDirection) = Nothing
    ...
    Public Sub New(ByVal inputPayload As Dictionary(Of Date, Payload),
        ...
        Dim filename As String = Path.Combine(My.Application.Info.DirectoryPath, "Nifty Banknifty Pair Trade List.csv")
        Using csvHelper As New Utilities.DAL.CSVHelper(filename, ",", _cts)
            dt = csvHelper.GetDataTableFromCSV(1)
        End Using
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            For i = 1 To dt.Rows.Count - 1
                Dim rowDate As Date = dt.Rows(i)(0)
                If rowDate.Date = _tradingDate.Date Then
                    Dim time As Date = dt.Rows(i)(1)
                    Dim signal As Integer = If(dt.Rows(i).Item(4).ToString.ToUpper = "LONG", 1, -1)
                    Dim instrumentName As String = _tradingSymbol.Remove(_tradingSymbol.Count - 8)
                    If instrumentName.ToUpper = "BANKNIFTY" Then
                        signal = signal * -1
                    End If
                    Dim signalDirection As Trade.TradeExecutionDirection = Trade.TradeExecutionDirection.None
                    If signal = 1 Then
                        signalDirection = Trade.TradeExecutionDirection.Buy
                    Else
                        signalDirection = Trade.TradeExecutionDirection.Sell
                    End If

                    If _tradeEntryTimeList Is Nothing Then _tradeEntryTimeList = New Dictionary(Of Date, Trade.TradeExecutionDirection)
                    _tradeEntryTimeList.Add(time, signalDirection)
                End If
            Next
        End If
    End Sub
    ...
    Public Overrides Async Function IsTriggerReceivedForPlaceOrderAsync(currentTick As Payload) As Task(Of Tuple(Of Boolean, List(Of PlaceOrderParameters)))
        ...
        If _tradeEntryTimeList IsNot Nothing AndAlso _tradeEntryTimeList.Count > 0 Then
            ...
            If currentMinuteCandlePayload IsNot Nothing AndAlso currentMinuteCandlePayload.PreviousCandlePayload IsNot Nothing AndAlso
                ...
                If signalCandle IsNot Nothing Then
                    If _tradeEntryTimeList.ContainsKey(signalCandle.PayloadDate) Then
                        Dim direction As Trade.TradeExecutionDirection = _tradeEntryTimeList(signalCandle.PayloadDate)
