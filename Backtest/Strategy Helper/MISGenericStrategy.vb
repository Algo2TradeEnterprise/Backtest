                            ...
                            If XDayOneMinutePayload IsNot Nothing AndAlso XDayOneMinutePayload.Count > 0 Then
                                ...
                                If currentDayOneMinutePayload IsNot Nothing AndAlso currentDayOneMinutePayload.Count > 0 Then
                                    ...
                                    Select Case RuleNumber
                                        ...
                                        Case 22
                                            stockRule = New LossMakeupNeutralSlabStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData, stockList(stock).Slab)
                                        Case 23
                                            stockRule = New NeutralSlabMartingaleStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData, stockList(stock).Slab)
                                        Case 24
                                        ...
                                        Case 35
                                            stockRule = New HigherTimeframeDirectionMartingaleStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 36
                                            stockRule = New SqueezeBreakoutStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 37
                                            stockRule = New MultiTradeLossMakeupStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 38
                                            stockRule = New MomentumReversalModifiedStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 39
                                            stockRule = New HKReverseExitStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 40
                                            stockRule = New BothDirectionMultiTradesHKStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 41
                                            stockRule = New MarketEntryStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 42
                                            stockRule = New HKReversalLossMakeupStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 43
                                            stockRule = New SupertrendCutStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 44
                                            stockRule = New BothDirectionMultiTradesStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 45
                                            stockRule = New DayHLSwingTrendlineStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 46
                                            stockRule = New FibonacciBacktestStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 47
                                        ...
                                        Case 55
                                            stockRule = New BollingerTouchStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 56
                                            stockRule = New AtTheMoneyOptionBuyOnlyStrategy(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData, stockList(stock).SupportingDate, stockList(stock).Supporting1)
                                        Case 57
                                            stockRule = New PreviousDayHKTrendSwingStrategy(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 58
                                        ...
                                        Case 75
                                            stockRule = New LowRangeCandleBreakoutStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 76
                                            stockRule = New LowRangeFirstCandleBreakoutStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case 77
                                            stockRule = New LowRangeFirstCandleBreakoutGapStrategyRule(XDayOneMinutePayload, stockList(stock).LotSize, Me, tradeCheckingDate, tradingSymbol, _canceller, RuleEntityData)
                                        Case Else
                ...
                If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                    Select Case Me.RuleNumber
                        ...
                        Case 25
                            ...
                            If stockDetails IsNot Nothing AndAlso stockDetails.Count > 0 Then
                                ...
                                For Each runningStock In stockDetails.OrderBy(Function(x)
                                    ...
                                    If Not ret.ContainsKey(runningStock.StockName) Then ret.Add(runningStock.StockName, runningStock)
                        ...
                        'Case 30
                            ...
                            '        Dim slab As Decimal = dt.Rows(i).Item("Slab")
                            '        Dim atr As Decimal = dt.Rows(i).Item("ATR %")
                            '        Dim crAtr As Decimal = dt.Rows(i).Item("CR Day ATR %")
                        ...
                        Case 31, 32
                            ...
                            If stockDetails IsNot Nothing AndAlso stockDetails.Count > 0 Then
                                ...
                                For Each runningStock In stockDetails.OrderByDescending(Function(x)
                                                                                            Return x.Supporting1
                                                                                        End Function)
                                    If runningStock.Supporting2 <= 60 Then
                                        If ret Is Nothing Then ret = New Dictionary(Of String, StockDetails)
                                        ret.Add(runningStock.StockName, runningStock)
                                ...
                                If rowDate.Date = tradingDate.Date Then
                                    ...
                                    Dim detailsOfStock As StockDetails = New StockDetails With
                                        {.StockName = instrumentName,
                                         ...
                                         .LotSize = lotsize,
                                         .Slab = slab,
                                         .Supporting1 = If(direction = "BULLISH", 1, -1),
                                         .Supporting2 = targetToStoplossMul,
                                         .EligibleToTakeTrade = True}
                        ...
                        Case 56
                            ...
                            For i = 0 To dt.Rows.Count - 1
                                ...
                                If rowDate.Date = tradingDate.Date Then
                                    ...
                                    End If
                                    Dim lotsize As Integer = dt.Rows(i).Item("Lot Size")
                                    Dim tradeEntryTime As Date = Date.ParseExact(dt.Rows(i).Item("Time"), "dd-MM-yyyy HH:mm:ss", Nothing)
                                    Dim turnoverRatio As Decimal = dt.Rows(i).Item("Turnover Ratio")

                                    Dim detailsOfStock As StockDetails = New StockDetails With
                                                {.StockName = instrumentName,
                                                 .TradingSymbol = tradingSymbol,
                                                 .LotSize = lotsize,
                                                 .SupportingDate = tradeEntryTime,
                                                 .Supporting1 = turnoverRatio,
                                                 .EligibleToTakeTrade = True}
                        ...
                        Case 69
                            ...
                            For i = 0 To dt.Rows.Count - 1
                                ...
                                If rowDate.Date = tradingDate.Date Then
                                    ...
                                    Dim slab As Decimal = dt.Rows(i).Item("Slab")
                                    Dim direction As String = dt.Rows(i).Item("Direction")
                                    Dim gap As Boolean = dt.Rows(i).Item("Gap")
                                    Dim detailsOfStock As StockDetails = New StockDetails With
                                                {.StockName = instrumentName,
                                                 .TradingSymbol = tradingSymbol,
                                                 .LotSize = lotsize,
