...
Namespace StrategyHelper
    Public Class CNCCandleGenericStrategy
        Inherits Strategy
                    ...
                    If stockList IsNot Nothing AndAlso stockList.Count > 0 Then
                        ...
                        For Each stock In stockList.Keys
                            ...
                            If Me.DataSource = SourceOfData.Database Then
                                XDayPayload = Cmn.GetRawPayload(Me.DatabaseTable, stock, tradeCheckingDate.AddDays(-10), tradeCheckingDate)
                            ElseIf Me.DataSource = SourceOfData.Live Then
                                XDayPayload = Await Cmn.GetHistoricalDataAsync(Me.DatabaseTable, stock, tradeCheckingDate.AddDays(-10), tradeCheckingDate).ConfigureAwait(False)
                            End If
                        ...
                        If currentDayStocksPayload IsNot Nothing AndAlso currentDayStocksPayload.Count > 0 Then
                            ...
                            While startMinute < endMinute
                                ...
                                For Each stockName In stockList.Keys
                                    ...
                                    currentDayStocksPayload(stockName).ContainsKey(potentialCandleSignalTime) Then
                                        currentMinuteCandlePayload = currentDayStocksPayload(stockName)(potentialCandleSignalTime)
                                    End If

                                    _canceller.Token.ThrowIfCancellationRequested()
                                    If currentMinuteCandlePayload IsNot Nothing Then
                                        _canceller.Token.ThrowIfCancellationRequested()
                                        SetCurrentLTPForStock(currentMinuteCandlePayload, currentMinuteCandlePayload, Trade.TypeOfTrade.CNC)

                                        'Update Collection
                                        Await stockStrategyRule.UpdateRequiredCollectionsAsync(currentMinuteCandlePayload).ConfigureAwait(False)
                                        ...
                                        If potentialRuleCancelTrades IsNot Nothing AndAlso potentialRuleCancelTrades.Count > 0 Then
                                            If Me.TickBasedStrategy OrElse Not stockList(stockName).CancelOrderDoneForTheMinute Then
                                                For Each runningCancelTrade In potentialRuleCancelTrades
                                                    _canceller.Token.ThrowIfCancellationRequested()
                                                    Dim exitOrderDetails As Tuple(Of Boolean, Decimal, String) = Await stockStrategyRule.IsTriggerReceivedForExitCNCEODOrderAsync(currentMinuteCandlePayload, runningCancelTrade).ConfigureAwait(False)
                                                    If exitOrderDetails IsNot Nothing AndAlso exitOrderDetails.Item1 Then
                                                        ...
                                                        Dim exitTick As Payload = New Payload(Payload.CandleDataSource.Calculated) With {
                                                                ...
                                                                .PayloadDate = currentMinuteCandlePayload.PayloadDate,
                                        ...
                                        If potentialRuleExitTrades IsNot Nothing AndAlso potentialRuleExitTrades.Count > 0 Then
                                            If Me.TickBasedStrategy OrElse Not stockList(stockName).ExitOrderDoneForTheMinute Then
                                                For Each runningExitTrade In potentialRuleExitTrades
                                                    _canceller.Token.ThrowIfCancellationRequested()
                                                    Dim exitOrderDetails As Tuple(Of Boolean, Decimal, String) = Await stockStrategyRule.IsTriggerReceivedForExitCNCEODOrderAsync(currentMinuteCandlePayload, runningExitTrade).ConfigureAwait(False)
                                                    If exitOrderDetails IsNot Nothing AndAlso exitOrderDetails.Item1 Then
                                                        _canceller.Token.ThrowIfCancellationRequested()
                                                        Dim exitTick As Payload = New Payload(Payload.CandleDataSource.Calculated) With {
                                                                .Open = exitOrderDetails.Item2,
                                        ...
                                        If Me.StockNumberOfTrades(currentMinuteCandlePayload.PayloadDate, currentMinuteCandlePayload.TradingSymbol) < Me.NumberOfTradesPerStockPerDay AndAlso
                                            Me.TotalPLAfterBrokerage(currentMinuteCandlePayload.PayloadDate) < Me.OverAllProfitPerDay AndAlso
                                            Me.TotalPLAfterBrokerage(currentMinuteCandlePayload.PayloadDate) > Math.Abs(Me.OverAllLossPerDay) * -1 AndAlso
                                            Me.StockPLAfterBrokerage(currentMinuteCandlePayload.PayloadDate, currentMinuteCandlePayload.TradingSymbol) < Me.StockMaxProfitPerDay AndAlso
                                            Me.StockPLAfterBrokerage(currentMinuteCandlePayload.PayloadDate, currentMinuteCandlePayload.TradingSymbol) > Math.Abs(Me.StockMaxLossPerDay) * -1 AndAlso
                                            Me.StockPLAfterBrokerage(currentMinuteCandlePayload.PayloadDate, currentMinuteCandlePayload.TradingSymbol) < stockStrategyRule.MaxProfitOfThisStock AndAlso
                                            Me.StockPLAfterBrokerage(currentMinuteCandlePayload.PayloadDate, currentMinuteCandlePayload.TradingSymbol) > Math.Abs(stockStrategyRule.MaxLossOfThisStock) * -1 Then
                                            placeOrderDetails = Await stockStrategyRule.IsTriggerReceivedForPlaceOrderAsync(currentMinuteCandlePayload).ConfigureAwait(False)
                                        ...
                                        If potentialModifySLTrades IsNot Nothing AndAlso potentialModifySLTrades.Count > 0 Then
                                            If Me.TickBasedStrategy OrElse Not stockList(stockName).ModifyStoplossOrderDoneForTheMinute OrElse Me.TrailingStoploss Then
                                                For Each runningModifyTrade In potentialModifySLTrades
                                                    _canceller.Token.ThrowIfCancellationRequested()
                                                    Dim modifyOrderDetails As Tuple(Of Boolean, Decimal, String) = Await stockStrategyRule.IsTriggerReceivedForModifyStoplossOrderAsync(currentMinuteCandlePayload, runningModifyTrade).ConfigureAwait(False)
                                        ...
                                        If potentialModifyTargetTrades IsNot Nothing AndAlso potentialModifyTargetTrades.Count > 0 Then
                                            If Me.TickBasedStrategy OrElse Not stockList(stockName).ModifyTargetOrderDoneForTheMinute Then
                                                For Each runningModifyTrade In potentialModifyTargetTrades
                                                    _canceller.Token.ThrowIfCancellationRequested()
                                                    Dim modifyOrderDetails As Tuple(Of Boolean, Decimal, String) = Await stockStrategyRule.IsTriggerReceivedForModifyTargetOrderAsync(currentMinuteCandlePayload, runningModifyTrade).ConfigureAwait(False)
                                                    If modifyOrderDetails IsNot Nothing AndAlso modifyOrderDetails.Item1 Then
                                                        _canceller.Token.ThrowIfCancellationRequested()
