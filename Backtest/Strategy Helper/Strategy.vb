...
#Region "Public Functions"
        ...
        Public Function IsAnyTradeOfTheStockStoplossReached(ByVal currentMinutePayload As Payload, ByVal tradeType As Trade.TypeOfTrade) As Boolean
            Dim ret As Boolean = False
            If TradesTaken IsNot Nothing AndAlso TradesTaken.Count > 0 AndAlso TradesTaken.ContainsKey(currentMinutePayload.PayloadDate.Date) AndAlso
                TradesTaken(currentMinutePayload.PayloadDate.Date).ContainsKey(currentMinutePayload.TradingSymbol) Then
                Dim completeTrades As List(Of Trade) = GetSpecificTrades(currentMinutePayload, tradeType, Trade.TradeExecutionStatus.Close)
                ...
                Try
                    ...
                    If allTradesData IsNot Nothing AndAlso allTradesData.Count > 0 Then
                        ...
                        If maxDrawUp <> Decimal.MinValue AndAlso maxDrawDown <> Decimal.MinValue Then
                            fileName = String.Format("PL {0},Cap {1},DrwUp {2},DrwDwn {3},{4}.xlsx", Math.Round(strategyOutputData.NetProfit, 0), Math.Round(maxCapital, 0), Math.Round(maxDrawUp, 0), Math.Round(maxDrawDown, 0), fileName)
                        Else
                        ...
                        Using excelWriter As New ExcelHelper(filepath, ExcelHelper.ExcelOpenStatus.OpenAfreshForWrite, ExcelHelper.ExcelSaveType.XLS_XLSX, _canceller)
