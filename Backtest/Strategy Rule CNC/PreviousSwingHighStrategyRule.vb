...
Public Class PreviousSwingHighStrategyRule
...
#End Region
    ...
    Public Overrides Async Function IsTriggerReceivedForPlaceOrderAsync(currentTick As Payload) As Task(Of Tuple(Of Boolean, List(Of PlaceOrderParameters)))
        ...
        If currentDayCandlePayload IsNot Nothing AndAlso currentDayCandlePayload.PreviousCandlePayload IsNot Nothing Then
            Dim signals As List(Of Tuple(Of Boolean, Decimal, Integer, Integer, String, Date, Date)) = GetSignalForEntry(currentDayCandlePayload)
            If signals IsNot Nothing AndAlso signals.Count > 0 Then
                For Each runningSignal In signals
                    ...
                    Dim parameter As PlaceOrderParameters = New PlaceOrderParameters With {
                                                                ...
                                                                .Supporting4 = runningSignal.Item7.ToString("dd-MMM-yyyy HH:mm:ss")
                                                            }
    ...
    Private Function GetSignalForEntry(ByVal candle As Payload) As List(Of Tuple(Of Boolean, Decimal, Integer, Integer, String, Date, Date))
        Dim ret As List(Of Tuple(Of Boolean, Decimal, Integer, Integer, String, Date, Date)) = Nothing
        ...
        If swing.SwingHigh < preSwing.SwingHigh Then
            ...
            If candle.High > swing.SwingHigh + buffer AndAlso candle.Open < swing.SwingHigh + buffer Then
                Dim entryPrice As Decimal = swing.SwingHigh + buffer
                If lastTrade Is Nothing Then
                    If ret Is Nothing Then ret = New List(Of Tuple(Of Boolean, Decimal, Integer, Integer, String, Date, Date))
                    ret.Add(New Tuple(Of Boolean, Decimal, Integer, Integer, String, Date, Date)(True, entryPrice, quantity, iteration, "First Trade", swing.SwingHighTime, preSwing.SwingHighTime))
                Else
                    ...
                    If swing.SwingHighTime <> lastSignalTime Then
                        If entryPrice < lastTrade.EntryPrice Then
                            If lastTrade.EntryPrice - entryPrice >= atr Then
                                ...
                                If ret Is Nothing Then ret = New List(Of Tuple(Of Boolean, Decimal, Integer, Integer, String, Date, Date))
                                ret.Add(New Tuple(Of Boolean, Decimal, Integer, Integer, String, Date, Date)(True, entryPrice, quantity, iteration, "Below last entry", swing.SwingHighTime, preSwing.SwingHighTime))
                            End If
                        Else
                            If ret Is Nothing Then ret = New List(Of Tuple(Of Boolean, Decimal, Integer, Integer, String, Date, Date))
                            ret.Add(New Tuple(Of Boolean, Decimal, Integer, Integer, String, Date, Date)(True, entryPrice, quantity, iteration, "(Reset) Above last entry", swing.SwingHighTime, preSwing.SwingHighTime))
                        End If
                    End If
                End If
            End If
